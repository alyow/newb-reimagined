...
uniform vec4 FogColor;
  ...
  float offsets = 1.0 / float(RESOLUTION) / 32.0;
   ...
   float lumR = luminance601(texture2D(TEXTURE_0, coord + float2(offsets, 0.0)).rgb);
   float lumL = luminance601(texture2D(TEXTURE_0, coord - float2(offsets, 0.0)).rgb);
   float lumD = luminance601(texture2D(TEXTURE_0, coord + float2(0.0, offsets)).rgb);
   float lumU = luminance601(texture2D(TEXTURE_0, coord - float2(0.0, offsets)).rgb);
