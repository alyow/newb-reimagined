...
float cloudDf(vec3 pos, float rain, float time, vec2 boxiness) {
  ...
  // round y
  n *= 1.0 - 1.9*smoothstep(boxiness.y, 2.0 - boxiness.y, 2.0*abs(pos.y-0.5));
...
vec4 renderCloudsRounded(
  ...
  col.rgb *= 1.0 - 0.8*night;
    ...
    #ifdef NL_CLOUD2_NIGHT_INTENSITY
    float op = col.a - d.y;
    op = col.a - d.x;
    
    op = NL_CLOUD2_NIGHT_INTENSITY;
...
vec4 renderClouds(vec2 p, float t, float rain, vec3 horizonCol, vec3 zenithCol, const vec2 scale, const float velocity, const float shadow) {
  ...
  // layer 2
  p = 2.8 * p.yx + vec2(7.6, 2.8) + t * 0.1;
  t *= 0.5;
  p += vec2(sin(t * 0.1), cos(t * 0.1)) * 0.2;
  ...
  vec4 col;
  col.a = a + c*(1.0-a);
  col.rgb = horizonCol + zenithCol.ggg;
  col.rgb = mix(col.rgb, mix(zenithCol, zenithCol.ggg, 0.5), shadow*mix(b, d, c));
